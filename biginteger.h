...
public:
  ...
  std::vector<int> &digits() { return digits_; }
  [[nodiscard]] const std::vector<int> &digits() const { return digits_; }
  [[nodiscard]] size_t length() const { return digits_.size(); }

  [[nodiscard]] std::string toString() const;

  static int radix() { return kRadix; }
  static int base() { return kBase; }
...
private:
  ...
  [[nodiscard]] bool isZero() const {
    return digits_.empty() || (length() == 1 && digits_[0] == 0);
  }
...
void BigInteger::absAdd(const BigInteger &bigint) {
  int overflow(0);
  for (size_t i(0); i < std::max(length(), bigint.length()) || overflow > 0;
       ++i) {
    if (i == digits_.size()) {
      digits_.emplace_back(0);
    }
    digits_[i] += (i < bigint.length() ? bigint.digits()[i] : 0) + overflow;
...
void BigInteger::absSub(const BigInteger &subtrahend) {
  int underflow = 0;
  for (size_t i(0); i < subtrahend.length() || underflow > 0; ++i) {
    digits_[i] -=
        underflow + (i < subtrahend.length() ? subtrahend.digits_[i] : 0);
...
void BigInteger::absMul(const BigInteger &multiplier) {
  ...
  composition.digits_.resize(length() + multiplier.length() - 1);
  long long overflow = 0;
  for (size_t rank(0); rank < composition.length(); ++rank) {
    long long column = overflow % radix();
    overflow /= radix();
    for (size_t i(
             std::max(static_cast<int>(rank - multiplier.length()) + 1, 0));
         i < std::min(rank + 1, length()); ++i) {
      column +=
...
void BigInteger::absDiv(const BigInteger &divider) {
  ...
  for (long long i(static_cast<long long>(length()) - 1LL); i >= 0; --i) {
...
std::string BigInteger::toString() const {
  ...
  for (long long i(static_cast<long long>(length()) - 2LL); i >= 0; --i) {
...
void BigInteger::removeLeadingZeros() {
  while (length() > 1 && digits_.back() == 0) {
    digits_.pop_back();
  }
  if (length() == 1 && digits_[0] == 0) {
    is_positive_ = true;
  }
}
